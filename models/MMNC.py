...
class MMNC:
    ...
    def run(self, G1, G2, _):
        ...
        if self.fast:
            ...
            train_dict = self.select_train_nodes(e1, e2, train_ratio=self.train_ratio)

        nodes1 = list(train_dict.keys())
        nodes2 = list([train_dict[i] for i in nodes1])
        # Step 2: Align Embedding Spaces
        aligned_embed1, embed2, trans_combined_e1, combined_e2 = \
            self.align_embedding(g1, g2, nodes1, nodes2, self.k_nei)
